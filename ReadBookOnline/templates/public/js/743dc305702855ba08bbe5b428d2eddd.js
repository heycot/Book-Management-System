...
Plugin.call($spy,data)})})}(jQuery);$(function(){var searchOpened=false;var lastWidth=0;function closeSearch(){if(searchOpened){$('.navbar-blue .navbar-brand').show();$('.navbar-blue .search-line').css('margin-left','');$('.navbar-blue .menu-open').css('display','');$('.navbar-blue .search-line-close').hide();searchOpened=false;}}
...
$('.xb-login-toggle-button').click();if($('.xb-login-inner form .form-group input').length>0){var field=$('.xb-login-inner form .form-group input').first().attr('id');$('.xb-login-inner form').yiiActiveForm('updateAttribute',field,[_this.splittedrater('getLoginMessage')]);}},200);});});/*!
 * jQuery.ellipsis
...
$(".item-cover img").one("load",function(){$(this).parents('.item-cover').find(".image.size-ad .inner-info .authors, .image.size-ad .inner-info .name").ellipsis({responsive:true});checkInfoVisibleAllow();recalculateShadows();}).each(function(){if(this.complete)$(this).load();});$(window).resize(function(){checkInfoVisibleAllow();recalculateShadows();});});$(function(){$('.xb-login-inner form').on('beforeSubmit',function(){var _this=$(this);var url=_this.attr('action');$.post(url,_this.serialize(),function(data){if(data.status=='success'){location.reload();}else if(data.errors){_this.yiiActiveForm('updateMessages',data.errors);}},'json');return false;});});yii.validation=(function($){var pub={isEmpty:function(value){return value===null||value===undefined||value==[]||value==='';},addMessage:function(messages,message,value){messages.push(message.replace(/\{value\}/g,value));},required:function(value,messages,options){var valid=false;if(options.requiredValue===undefined){var isString=typeof value=='string'||value instanceof String;if(options.strict&&value!==undefined||!options.strict&&!pub.isEmpty(isString?$.trim(value):value)){valid=true;}}else if(!options.strict&&value==options.requiredValue||options.strict&&value===options.requiredValue){valid=true;}
